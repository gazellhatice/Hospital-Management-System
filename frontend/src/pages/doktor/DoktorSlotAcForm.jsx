...
export default function DoktorSlotAcForm() {
  ...
  // --- Istanbul yerel tarih (YYYY-MM-DD)
  const today = useMemo(() => {
    const d = new Date();
  ...
  // --- en yakın 15 dk'ya yuvarla (örn. 14:07 -> 14:15)
  function roundToNextQuarter() {
    const now = new Date();
  ...
  function validate() {
    ...
    if (baslangic && bitis) {
      ...
      const [eh, em] = bitis.split(":").map(Number);
      const mins = eh * 60 + em - (sh * 60 + sm);
      if (mins < 10) next.bitis = "Süre en az 10 dakika olmalı.";
      if (mins > 8 * 60) next.bitis = "Süre en fazla 8 saat olabilir.";
    }
  ...
  async function handleSubmit(e) {
    ...
    if (!aktifDoktor || !(aktifDoktor.doktorId || aktifDoktor.id)) {
    ...
    const payload = {
      doktorId: Number(aktifDoktor.doktorId ?? aktifDoktor.id),
      tarih,
  ...
  const disabledAll = loading || !aktifDoktor || !(aktifDoktor.doktorId || aktifDoktor.id);
    ...
    <section className="bg-white/70 backdrop-blur-md border border-sky-100 rounded-3xl shadow-[0_8px_40px_-8px_rgba(14,165,233,0.15)] overflow-hidden transition-all duration-500 hover:shadow-[0_12px_50px_-10px_rgba(14,165,233,0.25)]">
      ...
      <header className="px-6 py-5 border-b border-sky-100 bg-gradient-to-r from-sky-50 via-white to-sky-50 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex flex-col">
          ...
          <h2 className="text-xl font-semibold text-slate-900 leading-tight">Slot Aç</h2>
          <p className="text-[13px] text-slate-600 leading-snug flex items-start gap-1">
            ...
            <span>Oluşturulan slotlar resepsiyon tarafından randevuya dönüştürülebilir.</span>
          </p>
        </div>
      ...
      {/* FORM */}
      <form className="px-6 pb-6 pt-4 grid gap-6 text-sm md:grid-cols-2" onSubmit={handleSubmit}>
        <div>
          <label className="block text-[13px] font-medium text-slate-700 mb-1">Tarih</label>
          <input
            type="date"
            value={tarih}
          ...
          />
          {fieldErr.tarih && <p className="mt-1 text-[12px] text-rose-600">{fieldErr.tarih}</p>}
        </div>

        <div>
          <label className="block text-[13px] font-medium text-slate-700 mb-1">Başlangıç Saati</label>
          <input
            type="time"
            value={baslangic}
        ...
        <div>
          <label className="block text-[13px] font-medium text-slate-700 mb-1">Bitiş Saati</label>
          <input
            type="time"
            value={bitis}
          ...
          />
          {fieldErr.bitis && <p className="mt-1 text-[12px] text-rose-600">{fieldErr.bitis}</p>}
        </div>

        <div className="md:col-span-2">
          <label className="block text-[13px] font-medium text-slate-700 mb-1">Açıklama (opsiyonel)</label>
          <input
            ...
            disabled={disabledAll}
          />
          <div className="mt-1 text-[11px] text-slate-400">{aciklama.length}/120</div>
        </div>
        ...
        <div className="md:col-span-2 flex flex-wrap items-center gap-3 rounded-xl border border-sky-100 bg-sky-50/50 px-3 py-2 text-[13px] text-slate-700">
          ...
          <span><b>Tarih:</b> {tarih || "—"}</span>
          <span>•</span>
          <span><b>Saat:</b> {baslangic || "--:--"} – {bitis || "--:--"}</span>
          <span>•</span>
          <span><b>Süre:</b> {durationText}</span>
        </div>
